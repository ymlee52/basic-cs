...
우리 프로그램이 `stdout`으로 보내는 건 그냥 바이트 스트림이다. 그걸 받아 화면에 그려주는 건 **별도 프로세스인 터미널 에뮬레이터**(Windows Terminal, iTerm2, gnome-terminal 등)다. 그래픽은 OS도 아니고 우리 코드도 아니고 터미널 프로그램의 일. 옛날 VT100 같은 단말기(화면 + 키보드만 있는 입출력 장치)에서 시작해 자라난 게 지금의 터미널 에뮬레이터.

터미널이 맡고 있는 일들:

1. **글자 그리기** — 가장 기본. ASCII/UTF-8 바이트를 화면에.
